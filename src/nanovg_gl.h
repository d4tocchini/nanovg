...
static int glnvg__renderCreate(void* uptr)
{
	...
	static const char* fillFragShader =
		...
		"#endif\n"
		"\n"
		"#define TYPE_FILLGRAD 				(type == 0) \n"
		"#define TYPE_FILLIMG 				(type == 1) \n"
		"#define TYPE_SIMPLE 				(type == 2) \n"
		"#define TYPE_IMG 					(type == 3) \n"
		"#define TYPE_FAST_FILLCOLOR 		(type == 5) \n" // 5-8 optimized draw modes
		"#define TYPE_FAST_FILLIMG 			(type == 6) \n"
		"#define TYPE_FILLCOLOR				(type == 7) \n"
  		"#define TYPE_FAST_FILLGLYPH 		(type == 8) \n"
		"\n"
		"void main(void) {\n"
		"   vec4 result;\n" // "	float strokeAlpha;	float scissor = scissorMask(fpos);\n"
		" 	if (TYPE_FAST_FILLCOLOR) {\n"
		"   	SET__strokeAlpha\n"
		"   	result = innerCol * strokeAlpha;\n"
		" 	} else if (TYPE_FAST_FILLIMG) {\n"
		"   	SET__strokeAlpha\n"
		"   	vec2 pt = (paintMat * vec3(fpos,1.0)).xy / extent;\n"
		"   	vec4 color = TEXTURE(tex, pt);\n"
		"   	if (texType == 1) color = vec4(color.xyz*color.w,color.w);"
		"   	result = innerCol * color * strokeAlpha;\n"
		" 	} else if (TYPE_FILLCOLOR) {\n"
		"   	SET__strokeAlpha\n"
		"   	vec4 color = innerCol;\n"
		"   	color *= strokeAlpha * scissorMask(fpos);\n"
		"   	result = color;\n"
		" 	} else if (TYPE_FAST_FILLGLYPH) {\n"
		"   	vec4 color = TEXTURE(tex, ftcoord);\n"
		"   	if (color.x < 0.02) discard;\n"
		"   	result = innerCol * color.x;\n"
		"	} else if (TYPE_FILLIMG) {\n" // Calculate color fron texture
		"   	SET__strokeAlpha\n"
		"		vec2 pt = (paintMat * vec3(fpos,1.0)).xy / extent;\n"
		"		vec4 color = TEXTURE(tex, pt);\n"
		"		if (texType == 1) color = vec4(color.xyz*color.w,color.w);"
